...
struct Boostlogger : ::testing::Test {
  void SetUp() { file_sink = boost::log::add_file_log("boost.log"); }
  void TearDown() {
...
TEST(FLV, put_test) {
  unsigned char mp4_nal[] = {0x00, 0x00, 0x00, 0x01};
  ...
  EXPECT_TRUE(std::equal(ts_nal_start_code.ptr_,
                         ts_nal_start_code.ptr_ + ts_nal_start_code.len_,
                         mp4_nal));
}

TEST(FLV, replace_nalu_start_code_from_mp4_to_ts) {
  unsigned char mp4_nal[] = {
      0x00, 0x00, 0x00, 0x02, 0xD9, 0x40, 0x00, 0x00, 0x00, 0x01,
      0xD9, 0x00, 0x00, 0x00, 0x0A, 0x01, 0x02, 0x03, 0x04, 0x05,
      0x06, 0x07, 0x08, 0x09, 0x0A,

      0x00, 0x00, 0x00, 0x10, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
      0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10};
  unsigned char ts_nal[] = {
      0x00, 0x00, 0x00, 0x01, 0xD9, 0x40, 0x00, 0x00, 0x00, 0x01, 0xD9, 0x00,
      0x00, 0x00, 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
      0x0A, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
  ...
  EXPECT_TRUE(
      std::equal(mp4_buffer.ptr_, mp4_buffer.ptr_ + mp4_buffer.len_, mp4_nal));

  replace_nalu_start_code_from_mp4_to_ts(4, mp4_buffer);

  EXPECT_TRUE(ts_nal_size == mp4_buffer.len_);
  EXPECT_TRUE(
      std::equal(mp4_buffer.ptr_, mp4_buffer.ptr_ + mp4_buffer.len_, ts_nal));
}
...
TEST(FLV, vector_insert) {
  ...
  unsigned char data[] = {0x00, 0x00, 0x00, 0x01, 0x04, 0x04, 0x04, 0x04,
...
TEST(FLV, to_string_buffer_t) {
  unsigned char data[] = {0xaf, 0x02, 0xf8, 0x3e, 0x01, 0x1d, 0x2b, 0x20};
...
TEST(FLV, get_aac_header_complex4) {
  ...
  // channel config : 3
  unsigned char flv_aac_seq_header_data[] = {0xaf, 0x00, 0xf8, 0x3e,
                                             0x01, 0x1d, 0x2b, 0x20};
  unsigned char* aac_seq_buffer = flv_aac_seq_header_data;
  std::size_t aac_seq_buffer_len =
      sizeof(aac_seq_buffer) / sizeof(unsigned char);
  uint32_t object_type;
  uint32_t sample_rate;
  uint32_t channel_count;

  EXPECT_TRUE(get_aac_header(aac_seq_buffer, object_type, sample_rate,
                             channel_count, aac_seq_buffer_len));
...
TEST(FLV, get_aac_header_complex3) {
  ...
  // sample rate : 36501 hz
  // channel config : 3
  unsigned char flv_aac_seq_header_data[] = {0xaf, 0x00, 0xf8, 0x26, 0x40};
  unsigned char* aac_seq_buffer = flv_aac_seq_header_data;
  std::size_t aac_seq_buffer_len =
      sizeof(aac_seq_buffer) / sizeof(unsigned char);
  uint32_t object_type;
...
TEST(FLV, get_aac_header_complex2) {
  ...
  // sample rate : 36501 hz
  // channel config : 3
  unsigned char flv_aac_seq_header_data[] = {0xaf, 0x00, 0x17, 0x80,
                                             0x47, 0x4a, 0xb8};
  unsigned char* aac_seq_buffer = flv_aac_seq_header_data;
  std::size_t aac_seq_buffer_len =
      sizeof(aac_seq_buffer) / sizeof(unsigned char);
  uint32_t obejct_type;
  uint32_t sample_rate;
  uint32_t channel_count;

  EXPECT_TRUE(get_aac_header(aac_seq_buffer, obejct_type, sample_rate,
                             channel_count, aac_seq_buffer_len));
...
TEST(FLV, get_aac_header) {
  unsigned char flv_aac_seq_header_data[] = {0xaf, 0x00, 0x11, 0x90,
                                             0x56, 0xe5, 0x00};
  unsigned char* aac_seq_buffer = flv_aac_seq_header_data;
  std::size_t aac_seq_buffer_len =
      sizeof(aac_seq_buffer) / sizeof(unsigned char);
...
TEST(FLV, is_aac_header) {
  unsigned char flv_aac_seq_header_data[] = {0xaf, 0x00, 0x11, 0x90,
                                             0x56, 0xe5, 0x00};

  unsigned char flv_aac_data[] = {0xaf, 0x01, 0xde, 0x04, 0x00, 0x4c, 0x61,
                                  0x76, 0x63, 0x35, 0x37, 0x2e, 0x31, 0x30,
                                  0x37, 0x2e, 0x31, 0x30, 0x30};

  unsigned char flv_aac_audio_data[] = {0xde, 0x04, 0x00, 0x4c, 0x61, 0x76,
                                        0x63, 0x35, 0x37, 0x2e, 0x31, 0x30,
                                        0x37, 0x2e, 0x31, 0x30, 0x30};
  ...
  std::size_t aac_audio_buffer_len =
      sizeof(aac_audio_buffer) / sizeof(unsigned char);
  buffer_t audioes(aac_audio_buffer_len);
  audio_frame_t audioframe;

  EXPECT_TRUE(
      get_audio(aac_audio_buffer, audioes, audioframe, aac_audio_buffer_len));
  EXPECT_TRUE(std::equal(audioes.ptr_, audioes.ptr_ + audioes.len_,
                         flv_aac_audio_data));
}

TEST(FLV, get_avc_info) {
  unsigned char flv_avc_seq_header_data[] = {
      0x17, 0x00, 0x00, 0x00, 0x00, 0x01, 0x64, 0x00, 0x28, 0xff, 0xe1,
      0x00, 0x29, 0x67, 0x64, 0x00, 0x28, 0xac, 0xd9, 0x40, 0x78, 0x04,
      0x4f, 0xde, 0x03, 0x6a, 0x02, 0x02, 0x02, 0x80, 0x00, 0x01, 0xf4,
      0x80, 0x00, 0x75, 0x30, 0x71, 0x10, 0x00, 0x09, 0x89, 0x68, 0x00,
      0x05, 0x16, 0x17, 0xaf, 0x7c, 0x07, 0x16, 0x86, 0x0a, 0x96, 0x01,
      0x00, 0x05, 0x68, 0xff, 0xb5, 0x25, 0x00};
  unsigned char flv_avc_sps_data[] = {
      0x67, 0x64, 0x00, 0x28, 0xac, 0xd9, 0x40, 0x78, 0x04, 0x4f, 0xde,
      0x03, 0x6a, 0x02, 0x02, 0x02, 0x80, 0x00, 0x01, 0xf4, 0x80, 0x00,
      0x75, 0x30, 0x71, 0x10, 0x00, 0x09, 0x89, 0x68, 0x00, 0x05, 0x16,
      0x17, 0xaf, 0x7c, 0x07, 0x16, 0x86, 0x0a, 0x96};
  unsigned char flv_avc_pps_data[] = {0x68, 0xff, 0xb5, 0x25, 0x00};

  unsigned char flv_avc_video_data_1[] = {
      0x17, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x09, 0x10,
      0x00, 0x00, 0x00, 0x29, 0x67, 0x64, 0x00, 0x28, 0xac, 0xd9, 0x40,
      0x78, 0x04, 0x4f, 0xde, 0x03, 0x6a, 0x02, 0x02, 0x02, 0x80, 0x00,
      0x01, 0xf4, 0x80, 0x00, 0x75, 0x30, 0x71, 0x10, 0x00, 0x09, 0x89,
      0x68, 0x00, 0x05, 0x16, 0x17, 0xaf, 0x7c, 0x07, 0x16, 0x86, 0x0a,
      0x96, 0x00, 0x00, 0x00, 0x04, 0x68, 0xff, 0xb5, 0x25};

  unsigned char flv_video_es_data[] = {
      0x00, 0x00, 0x00, 0x02, 0x09, 0x10, 0x00, 0x00, 0x00, 0x29, 0x67, 0x64,
      0x00, 0x28, 0xac, 0xd9, 0x40, 0x78, 0x04, 0x4f, 0xde, 0x03, 0x6a, 0x02,
      0x02, 0x02, 0x80, 0x00, 0x01, 0xf4, 0x80, 0x00, 0x75, 0x30, 0x71, 0x10,
      0x00, 0x09, 0x89, 0x68, 0x00, 0x05, 0x16, 0x17, 0xaf, 0x7c, 0x07, 0x16,
      0x86, 0x0a, 0x96, 0x00, 0x00, 0x00, 0x04, 0x68, 0xff, 0xb5, 0x25};

  unsigned char flv_avc_video_data_2[] = {
      0x27, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x09,
      0x30, 0x00, 0x00, 0x00, 0x16, 0x06, 0x01, 0x0e, 0x00, 0x00,
      0x03, 0x00, 0x04, 0x00, 0x00, 0x03, 0x00, 0x00, 0x76, 0x48,
      0x30, 0xa5, 0x73, 0x40, 0x80, 0x00, 0x00};

  unsigned char flv_video_es_data2[] = {
      0x00, 0x00, 0x00, 0x02, 0x09, 0x30, 0x00, 0x00, 0x00, 0x16, 0x06,
      0x01, 0x0e, 0x00, 0x00, 0x03, 0x00, 0x04, 0x00, 0x00, 0x03, 0x00,
      0x00, 0x76, 0x48, 0x30, 0xa5, 0x73, 0x40, 0x80, 0x00, 0x00};
  ...
  std::size_t buffer_len =
      sizeof(flv_avc_seq_header_data) / sizeof(unsigned char);
  buffer_t sps(65535);
  buffer_t pps(65535);
  uint8_t nal_startcode_len;
  EXPECT_TRUE(get_first_sps_and_first_pps_from_avc_sequence_header(
      buffer, sps, pps, nal_startcode_len, buffer_len));
  ...
  std::size_t flv_video_buffer_len =
      sizeof(flv_avc_video_data_1) / sizeof(unsigned char);
  buffer_t videoes(flv_video_buffer_len);
  video_frame_t videoframe;

  EXPECT_TRUE(
      get_video(flv_video_buffer, videoes, videoframe, flv_video_buffer_len));
  EXPECT_TRUE(
      std::equal(videoes.ptr_, videoes.ptr_ + videoes.len_, flv_video_es_data));
  EXPECT_EQ(1, videoframe.keyframe_);

  unsigned char* flv_video_buffer2 = flv_avc_video_data_2;
  std::size_t flv_video_buffer_len2 =
      sizeof(flv_avc_video_data_2) / sizeof(unsigned char);
  buffer_t videoes2(flv_video_buffer_len2);
  video_frame_t videoframe2;

  EXPECT_TRUE(get_video(flv_video_buffer2, videoes2, videoframe2,
                        flv_video_buffer_len2));
  EXPECT_TRUE(std::equal(videoes2.ptr_, videoes2.ptr_ + videoes2.len_,
                         flv_video_es_data2));
...
TEST(FLV, parse_avc) {
  ...
  unsigned char flv_avc_sps_data[] = {
      0x00, 0x00, 0x00, 0x01, 
      0x67, 0x64, 0x00, 0x28, 0xac, 0xd9, 0x40, 0x78, 0x04, 0x4f,
      0xde, 0x03, 0x6a, 0x02, 0x02, 0x02, 0x80, 0x00, 0x01, 0xf4,
      0x80, 0x00, 0x75, 0x30, 0x71, 0x10, 0x00, 0x09, 0x89, 0x68, 
      0x00, 0x05, 0x16, 0x17, 0xaf, 0x7c, 0x07, 0x16, 0x86, 0x0a, 0x96};
  ...
  EXPECT_FALSE(
      IsAVCNALU(flv_avc_seq_header_data,
                sizeof(flv_avc_seq_header_data) / sizeof(unsigned char)));
  EXPECT_TRUE(IsAVCNALU(flv_avc_video_data_1,
                        sizeof(flv_avc_video_data_1) / sizeof(unsigned char)));
  EXPECT_TRUE(IsAVCNALU(flv_avc_video_data_2,
                        sizeof(flv_avc_video_data_2) / sizeof(unsigned char)));

  EXPECT_TRUE(IsAVCSequenceHeader(
      flv_avc_seq_header_data,
      sizeof(flv_avc_seq_header_data) / sizeof(unsigned char)));
  EXPECT_FALSE(IsAVCSequenceHeader(
      flv_avc_video_data_1,
      sizeof(flv_avc_video_data_1) / sizeof(unsigned char)));
  EXPECT_FALSE(IsAVCSequenceHeader(
      flv_avc_video_data_2,
      sizeof(flv_avc_video_data_2) / sizeof(unsigned char)));

  {
    ...
    EXPECT_TRUE(ParseAVCSequenceHeader(buffer, video_data, avc_decoder_config,
                                      buffer_len));

    EXPECT_TRUE(std::equal(avc_decoder_config.sps_list_.begin(),
                          avc_decoder_config.sps_list_.end(), flv_avc_sps_data));
    EXPECT_TRUE(std::equal(avc_decoder_config.pps_list_.begin(), avc_decoder_config.pps_list_.end(), flv_avc_pps_data));

    std::cout << "flv_video_data["
              << video_data.ToString()
              << "],  flv_avc_decorder_config["
              << avc_decoder_config.ToString()
              << "]" 
              << std::endl; 
  }
  {
    ...
    flv::AVCVideoData video_data;
    EXPECT_TRUE(
        ParseAVCNALU(flv_video_buffer,video_data, videoes,flv_video_buffer_len));

    EXPECT_TRUE(
        std::equal(videoes.ptr_, videoes.ptr_ + videoes.len_,
        flv_video_es_data));
  } 
  {
    unsigned char* flv_video_buffer2 = flv_avc_video_data_2;
    std::size_t flv_video_buffer_len2 =
        sizeof(flv_avc_video_data_2) / sizeof(unsigned char);
    buffer_t videoes(flv_video_buffer_len2);
    flv::AVCVideoData video_data;

    EXPECT_TRUE(
      ParseAVCNALU(flv_video_buffer2,video_data, videoes,flv_video_buffer_len2));

    EXPECT_TRUE(std::equal(videoes.ptr_, videoes.ptr_ + videoes.len_,
                          flv_video_es_data2));
  }
...
TEST(FLV, write_n) {
  unsigned char data[] = {0x64, 0x00, 0x28, 0xAC, 0xD9,
                          0x40, 0x78, 0x04, 0x02, 0xFF};
  unsigned char double_data[] = {0x64, 0x00, 0x28, 0xAC, 0xD9, 0x40, 0x78,
                                 0x04, 0x02, 0xFF, 0x64, 0x00, 0x28, 0xAC,
                                 0xD9, 0x40, 0x78, 0x04, 0x02, 0xFF};
  unsigned char three_data[] = {0x64, 0x00, 0x28, 0xAC, 0xD9, 0x40, 0x78, 0x04,
                                0x02, 0xFF, 0x64, 0x00, 0x28, 0xAC, 0xD9, 0x40,
                                0x78, 0x04, 0x02, 0xFF, 0x64, 0x00, 0x28, 0xAC,
                                0xD9, 0x40, 0x78, 0x04, 0x02, 0xFF};
  ...
  EXPECT_TRUE(
      std::equal(dst_buffer.ptr_, dst_buffer.ptr_ + dst_buffer.len_, data));
  ...
  EXPECT_TRUE(std::equal(dst_buffer.ptr_, dst_buffer.ptr_ + dst_buffer.len_,
                         double_data));
...
TEST(FLV, read_n_byte_again_and_again) {
  unsigned char data[] = {0x64, 0x00, 0x28, 0xAC, 0xD9,
                          0x40, 0x78, 0x04, 0x02, 0xFF};
  unsigned char false_data[] = {0x64, 0x00, 0x28, 0xAC, 0xD9,
                                0x40, 0x78, 0x04, 0x02, 0xFE};
  size_t data_len = sizeof(data) / sizeof(unsigned char);
  ...
  EXPECT_TRUE(
      std::equal(buffer.ptr_, buffer.ptr_ + buffer.len_, dst_buffer.ptr_));
...
TEST(FLV, read3_bytes_read3_buffer) {
  ...
  uint32_t cts_2 = (composition_timestamp_offset.ptr_[0] << 16) |
...
TEST(FLV, read_bytes2) {
  ...
  EXPECT_FALSE(
      std::equal(tbuffer.ptr_, tbuffer.ptr_ + tbuffer.len_, false_data));
}
...
TEST(FLV, get2byte) {
  ...
  get_2byte(buffer, value);
  unsigned char b1 = (value & 0xff00) >> 8;
  unsigned char b2 = (value & 0x00ff);

  EXPECT_TRUE(00 == b1);
  EXPECT_TRUE(01 == b2);

  std::string str = to_hex(value);
  EXPECT_TRUE("00 01" == str);
}

TEST(FLV, ReadBits) {
  std::vector<std::uint8_t> buffer1{0xAC, 0x01, 0xF9};

  BitReader bitreader(buffer1);
  EXPECT_TRUE(1 == bitreader.ReadBits(1));
  EXPECT_TRUE(0 == bitreader.ReadBits(1));
  EXPECT_TRUE(1 == bitreader.ReadBits(1));
  EXPECT_TRUE(0 == bitreader.ReadBits(1));
  EXPECT_TRUE(1 == bitreader.ReadBits(1));
  EXPECT_TRUE(1 == bitreader.ReadBits(1));
...
TEST(FLV, is_aac_header_test_again) {
  unsigned char flv_aac_seq_header_data[] = {0xaf, 0x00, 0x11, 0x90,
                                             0x56, 0xe5, 0x00};
