...
namespace sna = serialnumberarithmetic;

RtmpPayloadParser::RtmpPayloadParser() : recv_chunk_size_(128) {}

RtmpPayloadParseResult::type RtmpPayloadParser::parse_payload(
    RtmpHeader_ptr header_ptr, std::istream& stream, size_t buf_size,
    size_t& readed_size) {
  ...
  parsing_info_map::const_iterator recv_info_iter =
      incomplete_infos_.find(cs_id);

  if (recv_info_iter == incomplete_infos_.end()) {
    recv_info = received_payload_info_ptr(new received_payload_info);
    recv_info->first_chunk_header =
        RtmpHeader_ptr(new RtmpHeader(header_ptr.get()));

    if (recv_info->first_chunk_header->has_msg_len_and_type()) {
      channels_[cs_id].last_msg_len =
          recv_info->first_chunk_header->msg_length_;
      channels_[cs_id].last_msg_type_id =
          recv_info->first_chunk_header->msg_type_id_;
    } else {
      recv_info->first_chunk_header->msg_length_ =
          channels_[cs_id].last_msg_len;
      recv_info->first_chunk_header->msg_type_id_ =
    ...
    recv_info->recv_buf =
        new unsigned char[recv_info->first_chunk_header->msg_length_];
    ...
    if (recv_info->first_chunk_header->format_type_ == RtmpHeaderFormat::FULL) {
      if (recv_info->first_chunk_header->timestamp_delta_ != 0) {
        RTMPLOGF(
            warning,
            "timestamp delta of message header(type0) is not zero. header[%1%]",
            recv_info->first_chunk_header->to_string());
      }
      ...
      if (channels_[cs_id].last_msg_timestamp_delta == 0) {
        RTMPLOGF(
            warning,
            "preceding timestamp delta is zero for new message. header[%1%]",
            recv_info->first_chunk_header->to_string());
      }
      // rollover
      {
    ...
    if (current_msg_stream_Id !=
        recv_info->first_chunk_header->msg_stream_id_) {
      RTMPLOG(debug) << "current message stream id:" << current_msg_stream_Id
                     << "current header info:" << header_ptr->to_string()
      ...
      for (size_t i = 0; i < len; i++) {
        oss << std::setw(2)
            << static_cast<unsigned int>(
                   recv_info->recv_buf[recv_info->recv_length + i])
            << " ";
      }
  ...
  if (this_msg_size == need_size) {
    ...
    if (first_header->is_control_msg()) {
      ...
      control_msg_parser_.parse_control_msg(
          first_header, payload_stream, msg_length, readed_in_payload_parse);
      set_parsed_msg(control_msg_parser_.get_parsed_msg());
    } else if (first_header->is_command_msg()) {
      size_t readed_in_payload_parse;
      if (!command_msg_parser_.parse_command_msg(first_header, payload_stream,
                                                 msg_length,
      ...
      size_t readed_in_payload_parse;
      if (!data_msg_parser_.parse_data_msg(first_header, payload_stream,
                                           msg_length, readed_in_payload_parse))
        return RtmpPayloadParseResult::FAIL;
      set_parsed_msg(data_msg_parser_.get_parsed_msg());
    } else if (first_header->is_video_msg() || first_header->is_audio_msg()) {
      size_t readed_in_payload_parse;
      if (!media_msg_parser_.parse_media_msg(first_header, payload_stream,
                                             msg_length,
                                             readed_in_payload_parse))
        return RtmpPayloadParseResult::FAIL;
  ...
  // msg incomplete : // message length 가  chunk size 보다 커서, 여러 chunk 를
  // 읽어야 하는 경우
  else {
