...
struct buffer_t {
 public:
  buffer_t(std::size_t max_len) : ptr_(new unsigned char[max_len]) {
    max_len_ = max_len;
  }
  ~buffer_t() { delete[] ptr_; }
...
std::string to_hex(std::vector<unsigned char>& value,
                   std::size_t showing_max_len = 32);
...
std::string to_hex(char* const buffer, std::size_t buffer_len,
                   size_t showing_max_len = 32);
...
struct BitReader {
  ...
  std::size_t BitsLeft() { return 8 * data_.size() - pos_; }
  std::uint32_t ReadBits(std::size_t n);
